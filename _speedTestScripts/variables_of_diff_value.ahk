...
rslts := "test1 time: "  test1 "`n"  "test2 time: " test2 "`n" "test3 time: " 
            ...
            . "test1 average time: " (test2+test5)/2 "`n"  "test3 average time: " (test3+test4)/2 "`n" 
...
QPC(R := 0)
{
    ...
    return ! DllCall("QueryPerformanceCounter", "Int64P", &Q) + (R ? 
                                                                 (P := Q) / F 
                                                                 : (Q - P) / F) 
}
...
f3()
{
    Loop 100
    {
        t3a := 1,t3b := 2,t3c := 3,t3d := 4
        ;,t3e := 5,t3f := 6,t3g := 7,t3h := 8,t3i := 9,t3j := 0
    }
}
